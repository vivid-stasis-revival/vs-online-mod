if (vs_lobby_lobby_id() > 0 && vs_lobby_is_owner())
{
    ...
    {
        suggestSong = _sq[0];
        var song = vs_online_song_from_id(suggestSong.songId);
        if (song == undefined)
        {
            var cid = "";
            if (variable_struct_exists(suggestSong, "chart_id")) cid = string(suggestSong.chart_id);
            song = vs_online_missing_song(cid);
        }
